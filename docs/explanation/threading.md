...
In NeoMutt, when a mailbox is sorted by *threads*, there are a few additional functions available in the *index* and *pager* modes as shown in the table below.
...
The tree should include at least one multi-level thread (3+ depth) with branching replies, a collapsed thread (showing the hidden message count), and a thread with a missing parent (showing the `?` character).
The tree-drawing characters (`├─>`, `└─>`, `│`) should be clearly visible.
...
Special characters will be added to the thread tree as detailed in the table below.
...
NeoMutt uses Unicode box-drawing characters to draw the thread tree (e.g., `├`, `└`, `│`, `─`).
If your terminal does not support Unicode, or you prefer plain ASCII, set the `$ascii_chars` boolean variable to `yes`.
This switches the tree characters to ASCII equivalents: `+`, `+`, `|`, and `-` respectively.
...
The `%<char?if-part&else-part>` syntax is explained in detail in the format string conditionals documentation.

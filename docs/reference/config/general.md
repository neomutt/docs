...
A colon-separated list of character encoding schemes for messages without character encoding indication.
...
When unset, you must first use the [`<tag-prefix>`](menu-generic) function (bound to {kbd}`;` by default) to make the next function apply to all tagged messages.
...
Controls whether NeoMutt will weed headers when invoking the [`<decode-copy>`](menu-index) or [`<decode-save>`](menu-index) functions.
...
Control the format of the date printed by the `%d` sequence in [`$index_format`](cfg-index-format).
...
Only used if [`$mime_forward`](cfg-mime-forward) is _unset_, otherwise [`$mime_forward_decode`](cfg-mime-forward-decode) is used instead.
...
Ignored if [`$use_from`](cfg-use-from) is _unset_.
...
Message statistics can also be explicitly calculated by invoking the [`<check-stats>`](menu-generic) function.
...
Used in connection with the [`<pipe-message>`](menu-attach) function.
...
For [`<pipe-message>`](menu-attach), when [`$pipe_decode`](cfg-pipe-decode) is set, this further controls whether NeoMutt will weed headers.
...
Used in connection with the [`<pipe-message>`](menu-attach) function following [`<tag-prefix>`](menu-generic).
...
Used in connection with the [`<print-message>`](menu-index) function.
...
For [`<print-message>`](menu-index), when [`$print_decode`](cfg-print-decode) is set, this further controls whether NeoMutt will weed headers.
...
Quoted text may be filtered out using the [`<toggle-quoted>`](menu-pager) command, or colored according to the `color quotedN` family of directives.
...
Not used if [`$from`](cfg-from) is set.
...
Can also be set via the [`<sort-mailbox>`](menu-index) and [`<sort-reverse>`](menu-browser) functions.
...
Set the time interval in seconds at which [`:timeout-hook`](cmd-timeout-hook) commands are executed while waiting for user input, either idling in menus or in an interactive prompt.
...
If this option is never set, then [`$sort`](cfg-sort) controls whether threading is used, [`$sort_aux`](cfg-sort-aux) controls both the sorting of threads and subthreads, and using [`<sort-mailbox>`](menu-index) to select threads affects only [`$sort`](cfg-sort).
Once this option is set, attempting to set [`$sort`](cfg-sort) to a value using "threads" will warn, the value of [`$sort`](cfg-sort) controls the sorting between threads while [`$sort_aux`](cfg-sort-aux) controls sorting within a thread, and [`<sort-mailbox>`](menu-index) toggles [`$use_threads`](cfg-use-threads).
...
Controls whether NeoMutt will ask you to press a key after an external command has been invoked by these functions: [`<shell-escape>`](menu-generic), [`<pipe-message>`](menu-attach), [`<pipe-entry>`](menu-attach), [`<print-message>`](menu-index), and [`<print-entry>`](menu-attach) commands.

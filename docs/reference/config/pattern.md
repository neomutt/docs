...
description: Config options for search patterns, external search commands, simple search expansion, and thorough search.
...
simple_search applies to several functions, e.g. [`<delete-pattern>`](menu-index), [`<limit>`](menu-alias), searching in the index, and all of the index colors.

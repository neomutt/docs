...
This setting is ignored if {ref}`$pager <pager>` is set.
...
If you use an _external_ {ref}`$pager <pager>`, setting this variable will cause NeoMutt to prompt you for a command when the pager exits rather than returning to the index menu.

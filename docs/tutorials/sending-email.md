...
description: Compose, reply, forward, and send messages using the compose menu, pseudo headers, PGP/S-MIME signing, and format=flowed
keywords: neomutt, sending email, compose, reply, forward, bounce, compose menu, pgp, smime, encrypt, sign, format flowed, outgoing mail, smtp, edit_headers, attach, fcc
...
If you haven't configured SMTP yet, start with [Writing Your First Configuration](first-config) or the guided [Start Here](../start-here) page.
...
Simple reply : Reply to the author directly.

Group reply : Reply to the author; cc all other recipients; consults `alternates` and excludes you.

Group Chat reply : Reply to the author and other recipients in the To list; cc other recipients in the Cc list; consults `alternates` and excludes you.
...
The compose menu is also used to edit the attachments for a message which can be either files or other messages.
The `<attach-message>` function will prompt you for a folder to attach messages from.
You can now tag messages in that folder and they will be attached to the message you are sending.
...
as a header, NeoMutt will pick up *filename* just as if you had used the `<edit-fcc>` function in the *compose* menu.
...
Spaces in filenames have to be escaped using backslash ("\\").
...
Otherwise, though you'll produce a technically valid reply, some netiquette guardians will be annoyed by this so-called "thread hijacking".

## Sending Cryptographically Signed/Encrypted Messages

If you have told NeoMutt to PGP or S/MIME encrypt a message, it will guide you through a key selection process when you try to send the message.
...
You can, as usually, abort this prompt using `^G`.

...
Using the [`$query_command`](cfg-query-command) variable, you specify the wrapper command to use.
For example:
...
:::{note}
The query menu is affected by [`$alias_sort`](cfg-alias-sort), thus overruling the order of entries as generated by [`$query_command`](cfg-query-command).
:::

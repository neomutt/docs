...
NeoMutt provides optional support for caching message headers for the following types of folders: IMAP, POP, Maildir and MH. 
...
For Maildir and MH, reading the headers from a single file is much faster than looking at possibly thousands of single files (since Maildir and MH use one file per message.)
...
Take a look at the benchmark script provided in the following repository: [contrib-hcache-benchmark](https://github.com/neomutt/contrib-hcache-benchmark). 
...
Pathnames are always stored in UTF-8 encoding.

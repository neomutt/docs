...
:::{note}
`reply-hook`s are matched *before* the `send-hook`, *regardless* of the order specified in the user's configuration file. However, you can inhibit `send-hook` in the reply case by using the pattern `'! ~Q'` (*not replied*) in the `send-hook` to tell when `reply-hook` have been executed.
:::
...
During key selection, NeoMutt will confirm whether each crypt-hook is to be used (unless the `$crypt_confirm_hook` option is unset). If all crypt-hooks for a recipient are declined, NeoMutt will use the original recipient address for key selection instead.

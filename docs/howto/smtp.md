...
1. Set the `$smtp_url` variable to the SMTP server URL. 
   If this is set, NeoMutt will contact the given SMTP server to deliver messages; if it is unset, NeoMutt will use the program specified by `$sendmail`.
...
3. Optionally configure the authentication mechanisms for SASL in `$smtp_authenticators`. 

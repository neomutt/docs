...
*map* is the map which the macro will be bound in. Multiple maps may be specified by separating multiple menu arguments by commas. Whitespace may not be used in between the menu arguments and the commas separating them.
...
*map* specifies from which menus the macro should be removed. Multiple menus may be specified by separating them with commas (no additional whitespace is allowed). If `*` is given, then the macro is removed from all menus. Valid menu names and their description are listed in the `bind` section.

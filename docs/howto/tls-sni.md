...
When non-secure URL protocols `imap://`, `pop://`, and `smtp://` are used, the initial connection to the server will be unencrypted. 
`STARTTLS` can be used to negotiate an encrypted connection after the initial unencrypted connection and exchange.

...
1. Back up your main config file from the location NeoMutt is using. See [Configuration File Locations](../reference/config/files).
2. Back up any files referenced with [`:source`](cmd-source), such as separate alias or account files.
...
2. Use [`:ifdef`](cmd-ifdef) or environment variables to handle machine-specific differences.

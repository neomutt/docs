...
with extraneous metadata that either is reproduced elsewhere, or is of
secondary interest.
...
`unsubject-regex` removes a given subject-regex from the substitution list. If
`*` is used as the argument, all substitutions will be removed.

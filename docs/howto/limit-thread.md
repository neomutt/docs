...
description: Use the limit-current-thread function to show only the current thread in the NeoMutt index
keywords: limit, thread, limit-current-thread, focus, filter, index, narrow view, conversation, thread view, restrict
---
...
**Description:** The NeoMutt index after invoking [`<limit-current-thread>`](fn-limit-current-thread), showing only the messages belonging to one thread (with tree-style indentation) while all other messages are hidden.
...
|-------|--------------------------|------------------------------|
| index | [`<limit-current-thread>`](fn-limit-current-thread) | Limit view to current thread |

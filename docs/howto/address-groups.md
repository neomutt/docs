...
The [`:group`](cmd-group) command is used to directly add either addresses or regular expressions to the specified group or groups.
The different categories of arguments to the [`:group`](cmd-group) command can be in any order.
...
Address groups can also be created implicitly by the [`:alias`](cmd-alias), [`:lists`](cmd-lists), [`:subscribe`](cmd-subscribe), and [`:alternates`](cmd-alternates) commands by specifying the optional `-group` option.
For example:
...
The syntax is similar to the [`:group`](cmd-group) command, however the special character `*` can be used to empty a group of all of its contents.
...
When removing regular expressions from a group, the *regex* must be specified exactly as given to the [`:group`](cmd-group) command or `-group` argument.

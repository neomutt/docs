...
1. Open the alias menu by pressing `Tab` at an address prompt (with an empty field) or by using the `<alias>` function if bound.
2. Press `/` to search.
...
4. Press `l` again and enter `~A` to clear the limit and show all aliases.
...
1. At an address prompt, press `^T` (`<complete-query>`).
